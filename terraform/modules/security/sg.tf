...
resource "aws_vpc_security_group_egress_rule" "web_http_to_internet" {
  security_group_id = aws_security_group.web.id
  description       = "HTTP to Ubuntu apt repos (signed packages, GPG-verified)"
  ip_protocol       = "tcp"
  from_port         = 80
  to_port           = 80
  cidr_ipv4         = "0.0.0.0/0"
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-web-egress-80-internet"
  }
}

resource "aws_vpc_security_group_egress_rule" "web_http_to_internet_v6" {
  security_group_id = aws_security_group.web.id
  description       = "HTTP to Ubuntu apt repos IPv6"
  ip_protocol       = "tcp"
  from_port         = 80
  to_port           = 80
  cidr_ipv6         = "::/0"
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-web-egress-80-internet-v6"
  }
}
