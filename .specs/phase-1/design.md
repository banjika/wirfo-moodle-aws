...
flowchart TB
    ...
    r53["Route 53<br/>academy.wirfocloud.com<br/>A + AAAA alias records"]
    acm["ACM cert (us-east-1)<br/>academy.wirfocloud.com<br/>DNS-validated"]
    ...
    ses["Amazon SES<br/>wirfocloud.com<br/>SPF + DKIM + DMARC"]
...
|---|---|
| Route 53 | Authoritative DNS for `wirfocloud.com`; A + AAAA alias to CloudFront |
...
- **Purpose:** Route 53 records, CloudFront distribution, and SES domain identity (DKIM/SPF/DMARC). The hosted zone for `wirfocloud.com` is **assumed to exist** and is read via a data source; it is not a variable. The operator verifies the zone manually before `terraform apply` (§8 step 2).
- **Key resources:**
  - `data "aws_route53_zone" "main" { name = "wirfocloud.com." }` — hardcoded zone name (trailing dot is intentional; reduces lookup ambiguity)
  ...
  - `aws_ses_domain_identity` for `wirfocloud.com`
  - `aws_ses_domain_dkim` (generates the three CNAME tokens AWS expects)
  - `aws_route53_record` × 3 — DKIM CNAMEs (`<token>._domainkey.wirfocloud.com → <token>.dkim.amazonses.com`)
...
| **DNS** | | | | |
| 12 | `domain_name` | `string` | `"academy.wirfocloud.com"` | Public-facing FQDN |
| 13 | `dmarc_rua_address` | `string` | `"dmarc@wirfocloud.com"` | RUA mailbox in the DMARC TXT record published by `modules/dns_cdn` |
| **Compute** | | | | |
...
- **Resources created:**
  ...
  - `aws_acm_certificate` in `us-east-1` for `academy.wirfocloud.com`, DNS validation
  - `data "aws_route53_zone" "main" { name = "wirfocloud.com." }` — same hardcoded data source as `dns_cdn`
...
| `domain_name` | `string` | `"academy.wirfocloud.com"` | Subject of the ACM cert |
...
- **Hosted zone:** `wirfocloud.com.` resolved via `data "aws_route53_zone"`; not a variable. Verified manually before apply.
